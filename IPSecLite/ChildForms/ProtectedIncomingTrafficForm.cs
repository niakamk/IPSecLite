...
{
    public partial class ProtectedIncomingTrafficForm : Form
    {
        delegate void ProtectedPacketArrivedHandler(object sender, ProtectedPacketArrivedEventArgs e);

        Dictionary<string, ProtectedPacketArrivedEventArgs> packets = new Dictionary<string, ProtectedPacketArrivedEventArgs>(500);

        TextBox[] replayWindow = new TextBox[16];

        public ProtectedIncomingTrafficForm()
        {
            InitializeComponent();
            ...
            for (int i = 0; i < APP_CONST.ANTI_REPLAY_WINDOW_SIZE; i++)
            {
                replayWindow[i] = new TextBox();
                this.replayWindowGroupBox.Controls.Add(replayWindow[i]);

                replayWindow[i].BorderStyle = System.Windows.Forms.BorderStyle.FixedSingle;
                replayWindow[i].Location = new System.Drawing.Point(60 + (i * 50), 20);
                replayWindow[i].ReadOnly = true;
                replayWindow[i].Size = new System.Drawing.Size(48, 20);
                replayWindow[i].TextAlign = System.Windows.Forms.HorizontalAlignment.Center;
            }
            resetAH();
            resetESP();

        }
        private void DrawReplayWindow(ushort Window, int WinRight)
        {

            for (int i = 0; i < APP_CONST.ANTI_REPLAY_WINDOW_SIZE; i++)
            {
                if (i == 0)
                    replayWindow[i].Text = WinRight.ToString();
                else
                    replayWindow[i].Text = (WinRight + i).ToString();
                if ((Window & (0x0001 << i)) != 0)
                {
                    replayWindow[i].ForeColor = Color.White;
                    replayWindow[i].BackColor = Color.Green;
                }
                else
                {
                    replayWindow[i].ForeColor = Color.Black;
                    replayWindow[i].BackColor = Color.White;
                }
            }
        }
        void displayProtectedPacketArrivedMessage(object sender, ProtectedPacketArrivedEventArgs e)
        {
            ...
            if (childSA != null)
            {
                ...
                switch (e.ProtectionResult)
                {
                    case PROTECTION_RESULTS.REPLAYED:
                        lvItem.BackColor = Color.OrangeRed;
                        break;
                    ...
                    case PROTECTION_RESULTS.INVALID_PAD:
                        lvItem.BackColor = Color.Chocolate;
                        break;
                };
                lvItem.SubItems.Add(e.ProtectionResult.ToString());
                if ((e.EncryptedLength - (e.MACLength + e.PaddingLength + 2)) > 0)
                    lvItem.SubItems.Add((e.EncryptedLength - (e.MACLength + e.PaddingLength + 2)).ToString());
                else
                ...
                DrawReplayWindow(childSA.ReplayWindow.replayWindow, childSA.ReplayWindow.right);
            }else
            {
                lvItem.BackColor = Color.Coral;
                lvItem.SubItems.Add("");
                lvItem.SubItems.Add("");
                lvItem.SubItems.Add("");
                lvItem.SubItems.Add(e.ProtectionResult.ToString());
        ...
        private void trafficListView_SelectedIndexChanged(object sender, EventArgs e)
        {
            ...
            {
                ProtectedPacketArrivedEventArgs packet;
                        ...
                        else
                        {
                            ...
                            switch (packet.ProtectionResult)
                            {
                                ...
                                case PROTECTION_RESULTS.OK:
                                    ahNextProtocolTextBox.Text = packet.NextProtocol.ToString();
                                    ahSPITextBox.Text = Utils.ToHexString(packet.SPI);
                                    //ahPayloadLengthTextBox.Text = packet.pa
                                    ahSequenceNumberTextBox.Text = packet.SequenceNumber.ToString();
                                    ahNextProtocolTextBox.Text = packet.NextProtocol.ToString();
                                    ahMacTextBox.Text = Utils.ToHexString(packet.MAC, false, 0);
                                    break;
